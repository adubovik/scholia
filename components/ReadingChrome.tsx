...
export function ReadingChrome({
  ...
  const { entries, leftOpen, drawerOpen, panelWidth, leftWidth, ready } = useNotesState();
  const { toggleDrawer } = useNotesActions();

  const style = {
    "--shift-left": leftOpen ? `${leftWidth}px` : "0px",
  ...
  return (
    <main className="page page--read reading-main" data-mode={mode} data-ready={ready} style={style}>
      {meta && (
