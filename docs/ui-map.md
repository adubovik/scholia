...
| the box you retype a paragraph in (node menu → "Edit text…" / "Add text…") | **edit-text sheet** (its `[phrase][1]` syntax = **bracket markup**) | `NodeEdit.tsx` → `EditTextSheet` (logic: `lib/annotations/markup.ts`) | `.node-sheet--wide`, `.node-sheet-text`, `.node-sheet-hint` |
...
| a `2.1₀` at the head of a node-note row **nested under its node** — same numbering, index 0 = the annotation covering the *whole* node. Appears **only** while a view band is on screen for that node: the band has taken the node's own row, so the note steps down beside the inline ones (`2.1₀`, then `2.1₁`, `2.1₂`…) | **node-note id** | `DualNodeSection.tsx` (`numberEl(sub)`), row made by `demote()` in `lib/tree/dual.ts` | `.node-num-id` + `.node-num-id sub` |
...
- **"Add text…" = the same item on a bodyless section.** A **bodyless section** is a node with only subsections and no prose of its own — its source range is zero-length, which is what the AI importer leaves behind when a chapter head's whole paragraph was its label (`ai-structure.ts::cutLength`). It renders as a bare id line, and the node menu's edit item reads "Add text…": the same **edit-text sheet**, opening blank, inserting what you write at that empty point (`updateNodeText` pins the node's own range so the insert doesn't just slide past it).
...
- The annotation tree is built by two pure functions in `lib/tree/dual.ts`: **`buildDual(tree, numbers)`** → the *full* `DualNode[]` (annotations arranged along the document hierarchy; a node's note is the prose, its inline annotations are **child rows**), and **`visibleDual(nodes, {filterTag, filterGlyphs, forceIds, layerIds})`** which prunes, filters and (see Views) restructures it at render time. Numbers come from the *reading* tree (a note on §2.1 is still 2.1 in both panels). Base pruning drops note-less leaves; a note-less node holding an annotated descendant survives as a **dim skeleton** (`.dual-skel`) — unless a selected view has text for it, which takes the slot instead (see Views below).
...
- **Choosing what shows:** the **view bar** (`LayerBar.tsx`) is a multi-select pill above the centred panel, `(original|summary|french)`. Selection lives in `LayerContext` and persists to `localStorage["scholia:layers"]`. Default: original on, no alternatives. Right-click a chip → rename / delete the view. The bar renders **nothing** until a document has its first view. The same chips show in **both** modes, `original` included — the reading panel is on screen either way (centred, or in the drawer behind the annotation tree), so that switch is live in both; it governs the reading panel only. In reading-first, switching `original` off hides the source passage on any node that *has* a selected view to read instead (`NodeSection`'s `hideOriginal`) — an untranslated node keeps its original rather than going blank, which also covers "every chip off". The first band then **takes the original's slot outright** (`leadsWithView`): it gets the run-in section number and sets in the same justified column as `SourcePassage`, so a view holding a copy of the original reads identically to it, tint aside. (Collapsed, there is no prose to lead — the number falls back to a head line.)
- **Rendering:** `LayerBands.tsx` renders the selected views' text as tinted bands — under the passage in reading-first (`NodeSection`), under the note/skeleton in annotation-first (`DualNodeSection`). The same **first-band-takes-the-slot** swap applies in both panels: reading-first when `original` is off, annotation-first whenever a skeleton's dim rundown could be replaced by a selected view that actually has text for that node (`leadsWithView` in each — titled headings keep their label). `LayerContext.useVisibleLayers(nodeId, layerNotes)` is the single test for "does a view render here" — selected **and** holding text (or mid-compose) — and all three call sites use it: the bands themselves, `hideOriginal`, `leadsWithView`. A node note and a band both want the passage's slot, so while a band shows the note **steps down a level**: `visibleDual`'s `demote()` hands the node's row to the band (leaving a skeleton) and re-emits the note as a `kind: "note"` child row numbered `2.1₀`, ahead of the inline `2.1₁`, `2.1₂` — keyed by the annotation's own id, so a cross-panel select lands on the note, not the band. Alone (no band) the note keeps the node's row and its bare number; a *titled* node is never demoted, since its row is a heading rather than a rundown. The tint is layout-neutral by construction: horizontal bleed only, cancelled by an equal negative margin, so the wash marks the text's role without moving a line. Bands carry **no name label**: the tint is the identifier, matched to the bar chip's fill (the name is a `title` on hover). Stacked bands glue into one block — the group rounds at its outer corners only, with a hairline between neighbours. A view with no text for that node is skipped (no empty bands). **Bands take no highlights and no comments** — annotations anchor to source offsets, and a rendition is not the source; only the original passage has `SourcePassage`.
...
| "Edit text…" is greyed out; a passage edit is refused ("missing"/"more than once"/"no highlight") | `lib/annotations/markup.ts` (`isMarkupEditable` gates the item, `fromMarkup` refuses the save) — greying means overlapping highlights only; an empty section reads "Add text…" instead |
