...
- **Choosing what shows:** the **view bar** (`LayerBar.tsx`) is a multi-select pill above the centred panel, `(original|summary|french)`. Selection lives in `LayerContext` and persists to `localStorage["scholia:layers"]`. Default: original on, no alternatives. Right-click a chip → rename / delete the view. The bar renders **nothing** until a document has its first view; in annotation-first it drops the `original` chip (that slot is the note, and there is no original prose to switch off). In reading-first, switching `original` off hides the source passage on any node that *has* a selected view to read instead (`NodeSection`'s `hideOriginal`) — an untranslated node keeps its original rather than going blank, which also covers "every chip off". The first band then **takes the original's slot outright** (`leadsWithView`): it gets the run-in section number and sets in the same justified column as `SourcePassage`, so a view holding a copy of the original reads identically to it, tint aside. (Collapsed, there is no prose to lead — the number falls back to a head line.)
