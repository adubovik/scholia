...
| a `2.1₀` at the head of a node-note row — same numbering, index 0 = the annotation covering the *whole* node. Appears **only** while a view band is on screen for that node, since that is the only time something else is also standing in for the passage | **node-note id** | `DualNodeSection.tsx` (`numberEl(sub)`) | `.node-num-id` + `.node-num-id sub` |
