...
How a reader sees that a node is annotated and reads that annotation, and how an editor restructures the tree and manages notes — via a marginal `¶` marker, a per-node context menu (with a `⋮` discoverability hint), a document-level menu on empty reading space, and keyboard shortcuts — without placing any control chrome on the prose body (which is reserved for the text-selection annotation gesture).
